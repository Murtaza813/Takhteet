...
def generate_schedule(start_juz, days_in_month):
    """Generate schedule data for PDF"""
    schedule = {}
    ...
    if daily_amount == "Mixed (0.5 & 1 page)":
        ...
        for i in range(working_days):
            ...
            schedule.append({'page': round(current_page_val, 1), 'amount': amount})
    else:
        amount = 0.5 if "0.5" in daily_amount else 1.0
        current_page_val = start_page
        for i in range(working_days):
            schedule.append({'page': current_page_val, 'amount': amount})
            if is_backward:
...
def create_pdf(student_name, selected_month_name, selected_year, start_juz, days_in_month):
    try:
        ...
        col_widths = [
            ...
            total_width * 0.12, total_width * 0.12, total_width * 0.11, total_width * 0.10
        ]
        
        if use_arabic:
            headers = [
                format_arabic("ملاحظات"), format_arabic("هدف حاصل كيڈو؟"),
                format_arabic("أجزاء المراجعة"), format_arabic("الجزء الحالي"),
                format_arabic("الجديد (صفحة رقم)"), format_arabic("كمية الجديد"),
                format_arabic("التاريخ")
            ]
        else:
            headers = ["Notes", "Target Achieved?", "Murajjah Parts", "Current Juzz", "New (Page No.)", "Amount", "Date"]
        ...
        for day_schedule in schedule_data:
            ...
            if is_holiday:
                cell_data = ["", "", format_arabic("عطلة") if use_arabic else "Holiday", "", "", "", str(day)]
            else:
                ...
                cell_data = ["", "", murajjah_clean, clean_juzz_hali, page_number, amount, str(day)]
...
def render_manual_murajjah_section():
    if st.session_state.murajjah_option == "Manual Selection":
        ...
        if st.session_state.show_manual_murajjah:
            st.markdown('<div class="murajjah-container">', unsafe_allow_html=True)
            st.markdown("#### 📋 Select Siparas for Each Day")
            st.caption("Click on sipara numbers to select/deselect for each day")
...
def main():
    with st.sidebar:
        st.markdown("<div style='text-align: center; margin-bottom: 2rem;'><h1>📖 Quran Hifz<br/>Takhteet</h1></div>", unsafe_allow_html=True)
        st.markdown("---")
        ...
        with st.expander("📋 How to Use", expanded=True):
            ...
            **Quick Start Guide:**
            ...
            2. 📅 Select month & year
            3. 🎯 Choose direction (Backward/Forward)
            4. 📄 Set page numbers
        ...
        with st.expander("✨ Features"):
            ...
            • **Auto Murajjah Generation**
            • **Manual Sipara Selection**
            • **Holiday Management**
        ...
        st.markdown("---")
        st.markdown("<div style='text-align: center; color: rgba(255,255,255,0.7);'>Made with ❤️ for Huffaz</div>", unsafe_allow_html=True)
    
    st.markdown('<div class="header-container"><h1>🌙 Quran Hifz Takhteet Generator</h1><p class="header-subtitle">Plan your journey to memorizing the Quran</p></div>', unsafe_allow_html=True)
    
    st.markdown('<div class="modern-card">', unsafe_allow_html=True)
    st.markdown('<h3 class="section-title">🎯 Hifz Direction</h3>', unsafe_allow_html=True)
    
    direction_cols = st.columns(2)
    with direction_cols[0]:
        if st.button("⬅️ Backward (30 → 1)", 
                    type="primary" if st.session_state.direction == "Backward (30 → 1)" else "secondary",
                    use_container_width=True):
            st.session_state.direction = "Backward (30 → 1)"
            st.rerun()
    with direction_cols[1]:
        if st.button("Forward (1 → 30) ➡️", 
    ...
    with col1:
        st.session_state.student_name = st.text_input(
            ...
            help="Name of the student memorizing Quran"
        )
        ...
        st.session_state.month = st.selectbox(
            ...
            index=11,
            help="Select the month for this schedule"
        )
        ...
        st.session_state.current_sipara = st.slider(
            ...
            max_value=30,
            value=21,
            help="Currently memorizing sipara number"
        )
    ...
    with col2:
        ...
        st.session_state.daily_amount = st.selectbox(
            ...
            index=2,
            help="How much to memorize each day"
        )
        ...
        st.session_state.extra_holidays = st.number_input(
            ...
            value=4,
            step=1,
            help="Additional days off at end of month"
        )
        ...
        st.session_state.murajjah_option = st.selectbox(
            ...
            index=2,
            help="How to assign revision siparas"
        )
    ...
    if st.session_state.schedule:
        ...
        st.markdown(f'<div class="modern-card"><h2 class="section-title">📅 Monthly Schedule</h2><h3 style="color: #64748b;">{st.session_state.student_name} - {month_name} {st.session_state.year}</h3><p style="color: #10b981; font-weight: 600;">Direction: {st.session_state.direction}</p></div>', unsafe_allow_html=True)
        ...
        st.dataframe(display_df.style.apply(highlight_holidays, axis=1), use_container_width=True, height=600)
